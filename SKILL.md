...
NODE: drawingInteraction
  ...
  key_snippet (optional):
    ...
    lang: typescript
  functions:
    - handlePointerDown: Listens for initial mouse/touch events and captures start position
    - handlePointerMove: Tracks pointer movement and creates element updates in real-time
    - handlePointerUp: Finalizes the shape and commits it to the element store
```
...
**Key requirement**: For each node, list the important functions/methods (2-5 max) with a brief one-line explanation of what each function does. These help developers understand the key operations within that concept.
...
nodeId: {
  ...
  lang: "typescript",
  // List important functions within this node with brief explanations
  functions: [
    { name: "handlePointerDown", explanation: "Captures initial pointer position and starts a new drawing session" },
    { name: "handlePointerMove", explanation: "Updates the shape in real-time as the user drags" },
    { name: "handlePointerUp", explanation: "Commits the completed shape to the element store" },
  ],
}
```
...
- `functions` = Array of 2-5 key functions/methods within the node. Each with a brief one-line explanation of what the function does. This helps developers understand the key operations.
...
**After writing the file**, provide the user with a command to open it in their browser:
```bash
open walkthrough-{topic}.html    # macOS
xdg-open walkthrough-{topic}.html # Linux
```

Do NOT automatically open the browser. Always show the command to the user instead.
...
- [ ] Each node includes **2-5 important functions** with brief one-line explanations
...
- [ ] Clicking any node shows its detail panel with functions listed
- [ ] The diagram renders without Mermaid syntax errors
- [ ] The HTML file was NOT automatically opened (user is given the command instead)
